...
public class TurtleTreasureHunt implements KeyEventDispatcher {
	...
	int tortoiseSpeed2 = -5;

	private void goDown() {
		// 2. make the tortoise move down the screen
		Tortoise.move(tortoiseSpeed2);
	}
	...
	private void spaceBarWasPressed() {
		...
		if (tortoiseLocationX == 505 && tortoiseLocationY == 300) {
			String A = JOptionPane.showInputDialog("Ask The girl what she wants");

			if (A.equals("What do you want?")) {

				JOptionPane.showMessageDialog(null, "Free me, Go to the boy and tell him to free me");
				if (tortoiseLocationX == 285 && tortoiseLocationY == 145) {
					String B = JOptionPane.showInputDialog("Yo Wassup homie. You need anything?");
					if (B.equals("Could you free the girl")) {
					}
				}
			}
